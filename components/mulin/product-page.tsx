        ...
        <section className="mx-auto max-w-[1320px] px-[clamp(20px,5vw,80px)] py-[clamp(56px,7vw,96px)]">
          ...
          >
            ...
            <motion.div className="relative" variants={rise}>
              <div className="overflow-hidden rounded-[44px] bg-[#FFFFFF] shadow-[0_18px_44px_rgba(18,26,20,0.06)]">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                {content.features.map((feature, index) => (
                  <div key={feature.title} className="rounded-[28px] bg-[#FFFFFF] px-4 py-4 shadow-[0_12px_30px_rgba(18,26,20,0.04)]">
                    <p className="text-[10px] uppercase tracking-[0.2em] text-[#7f8a79]">0{index + 1}</p>
                    <p className="mt-2 font-serif font-light text-[clamp(1rem,1.2vw,1.2rem)] leading-[1.05] text-[#454C23]">{feature.title}</p>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div className="mt-10 grid gap-6 md:grid-cols-3" initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.16 }} variants={container}>
              {content.process.map((item, index) => (
                <motion.article
                  ...
                  className="rounded-[36px] bg-[#FFFFFF] p-6 shadow-[0_16px_36px_rgba(18,26,20,0.05)]"
                >
                  <p className="text-[11px] uppercase tracking-[0.2em] text-[#7f8a79]">{item.step}</p>
                  <h3 className="mt-4 font-serif font-light text-[clamp(1.55rem,2vw,2.2rem)] leading-[1.02] text-[#454C23]">
                    {item.title}
                  </h3>
            ...
            <motion.div className="grid gap-4 sm:grid-cols-2" variants={container}>
              {content.outcomes.map((outcome, index) => (
                <motion.article
                  ...
                  className={`rounded-[34px] p-6 shadow-[0_16px_36px_rgba(18,26,20,0.05)] ${index === 0 ? 'sm:col-span-2 bg-[#454C23] text-[#FFFFFF]' : 'bg-[#FFFFFF]'
                    }`}
                >
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full ${index === 0 ? 'bg-white/12 text-white' : 'bg-[#eef2ea] text-[#454C23]'}`}>
                    <CheckIcon className="h-5 w-5" />
                  </div>
                  <h3 className={`mt-5 font-serif font-light text-[clamp(1.55rem,2vw,2.2rem)] leading-[1.02] ${index === 0 ? 'text-white' : 'text-[#454C23]'}`}>
                    {outcome.title}
                  </h3>
